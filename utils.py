...
def parse_datetime_with_timezone(datetime_str):
    try:
        ...
        logging.error(f"Invalid date string: {datetime_str}")
        return None
...
def send_email_confirmation(mail, guest, receipt_id, reservation_details, reservation_type, is_online=True):
...
def search_guests():
    ...
    # Query the database for guests that match the guest_client field
    guests = GuestDetails.query.filter(GuestDetails.guest_client.ilike(f'%{search_query}%')).all()
    # Format the guests to return in JSON
    guests_list = [
        {
    ...
    # Return the list of matching guests
